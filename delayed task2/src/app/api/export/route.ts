...
function sanitizeSheetTitle(raw: string) {
  ...
  // Google Sheets limits to 100 chars; keep a buffer
...
async function ensureUniqueSheetTitle(
  ...
  baseTitle: string
): Promise<{ title: string; existingTitles: Set<string> }> {
  ...
  if (!titles.has(baseTitle)) return { title: baseTitle, existingTitles: titles };

  // Append (2), (3), ... until unique
  for (let i = 2; i < 1000; i++) {
    const candidate = `${baseTitle} (${i})`.slice(0, 100);
    if (!titles.has(candidate)) return { title: candidate, existingTitles: titles };
  }
  // Fallback to timestamped title
  const ts = new Date().toISOString().replace(/[:.]/g, '-');
  return { title: `${baseTitle} ${ts}`.slice(0, 100), existingTitles: titles };
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!spreadsheetId) {
      // Fallback: create a new spreadsheet (note: may hit quota if Drive is full)
    ...
    // Prepare rows:
    // Row1: headers; Row2: username/date; Row3+: chatPairs user/bot
    const values: (string[])[] = [
      ['user name', 'date'],
      [username, exportDate],
    ...
    if (configuredSpreadsheetId) {
      // === Append as a NEW TAB on the existing spreadsheet ===
    ...
    // === Created new file path (fallback) ===
    // Write into default Sheet1
    await sheets.spreadsheets.values.update({
    ...
    // Freeze & autosize on sheetId=0 (first/default sheet)
    await sheets.spreadsheets.batchUpdate({
      spreadsheetId: spreadsheetId!,
      requestBody: {
        requests: [
